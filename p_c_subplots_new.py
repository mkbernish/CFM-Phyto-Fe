...
for j in range(np.size(namesforloop)):
    ...
    Pthylakoid=Chl*Ypthylakoid_chl    #array, e-18       #(molP cell-1) Phosphorus in thylakoid membranes: phospholipid, etc. (193-26)
    Prna=Nrna*YnucacidP_N  #array, e-17   #(molP cell-1) Phosphorus in RNA 
    Pessential=Pthylakoid+Prna+Pdna+Pconst_other #array, e-17   #(molP cell-1)
    ...
    Nconst_protein_plot=Nconst_protein*Nunit*Numbertoarray #array, .05    #(ug N/ mgC) constant protein pool in nitrogen (193-25)(193-33)
    ...
    Ndna_const_plot=Ndna_const*Nunit*Numbertoarray #array, 0.0003 (values are same)   #(ug N/ mgC) Nitrogen in constant part of DNA
    ...
    #For P/C
    #----------------------------------
    Pconst_other_plot=Pconst_other*Punit*Numbertoarray #array, 0.00065 (all same)      #(ug P/ mgC) PHosphorus in other parts (ex. phospholipid in outer membrane, ATP, ADP, etc. (assuming constant) (193-33)
    Pthylakoid_plot=Pthylakoid*Punit #array, 9e-5    #(ug P/ mgC) Phosphorus in thylakoid membranes: phospholipid, etc. (193-26)(193-33)
    Pdna_const_plot=Pdna_const*Punit*Numbertoarray  #array, 8.47 e-5 (all same)  #(ug P/ mgC) Phosphorus in constant part of DNA
    Prna_plot=Prna*Punit*Numbertoarray  #array, 2e-4  #(ug P/ mgC) Phosphorus in constant part of RNA
    Pstore_plot= zeros(size(Fe_high_res)) #(ug P/ mgC) Phosphorus in phosphorus storage
    # Pstore*Punit    
    ...
    StackPlotColorsP2=(Color_Other,Color_Photo,Color_RNA,Color_Nstore,Color_other)


    axs[j].stackplot(Fe_high_res*1000,Pconst_other_plot+Pdna_const_plot,Pthylakoid_plot,Prna_plot,Pstore_plot,colors=StackPlotColorsP2)

    axs[j].set_xlim(left=0,right=fe_max*1000)
    
    axs[j].set_ylim(bottom=0,top=0.007)

    
fss = 24
fig.text(0.5, 0.005, 'Fe (nmol L$^{-1}$)', ha='center')
fig.text(0.002, 0.5, "P:C (mol mol$^{-1}$)", va='center', rotation='vertical')


# for 10 subplots
fig.text(0.18,0.92,'a',fontsize=fss)
fig.text(0.38,0.92,'b',fontsize=fss)
fig.text(0.568,0.92,'c',fontsize=fss)
fig.text(0.766,0.92,'d',fontsize=fss)
fig.text(0.96,0.92,'e',fontsize=fss)
fig.text(0.18,0.432,'f',fontsize=fss)
fig.text(0.38,0.432,'g',fontsize=fss)
fig.text(0.568,0.432,'h',fontsize=fss)
fig.text(0.766,0.432,'i',fontsize=fss)
fig.text(0.96,0.432,'j',fontsize=fss)

sf_opt("fe_p_c_plot_709")
